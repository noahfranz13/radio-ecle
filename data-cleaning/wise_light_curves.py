...
def download_single_data(name, ra, dec, root_path='ipac/', radius=2.5, overwrite=False):
    #ra, dec: in degree
    #name, ra, dec = row['Name'], row['RAJ2000'], row['DEJ2000']
    #name = 'J' + ra + dec
    if root_path[-1] != '/':
        root_path += '/'
    if not overwrite and os.path.isfile(root_path+name+'_allwise.ipac') and os.path.isfile(root_path+name+'_neowise.ipac'):
        pass
    else:
...
def only_good_data_v1(allwise, neowise):
    ...
    This filtering is described here: https://wise2.ipac.caltech.edu/docs/release/neowise/expsup/sec2_3.html
    """
